...
function renderProducts(){const tb=document.querySelector('#productTable tbody');tb.innerHTML='';if(!data.products.length){tb.innerHTML='<tr><td colspan="8" class="empty">Chưa có sản phẩm. Bấm “Thêm sản phẩm” để bắt đầu.</td></tr>';return}data.products.forEach(p=>{const c=calc(p),tr=document.createElement('tr');tr.innerHTML=`<td><span class="product-code">${esc(p.code)}</span></td>
<td><span class="product-name">${esc(p.name)}</span></td>
<td class="num"><span class="cost-price">${money(c.cost)}</span></td>
<td class="num"><span class="sale-price direct">${money(c.direct)}</span></td>
<td class="num"><span class="sale-price social">${money(c.social)}</span></td>
<td class="num"><span class="sale-price shopee">${money(c.shopee)}</span></td>
<td class="num"><span class="sale-price tiktok">${money(c.tiktok)}</span></td>
